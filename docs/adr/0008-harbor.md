...
resolution (`config-object-store.yaml`). There's no new ADR-level dependency here: it's the
same config `addon-image-factory.yaml` already reads. The SSO step depends on
[ADR-0002](0002-identity.md) (the platform Keycloak realm), just as
...
shipped without a formal ADR of its own. This ADR picked Harbor over deepening Omni's own
...
`registry.harbor.*`, not a flat `harbor.*` key: capability-plus-driver, matching the pattern
...
directory is already structured to hold. The Kubernetes namespace is `registry`, for the same
...
needed), echoing the relationship Core's own `identity` flux system has to the keycloak-specific
pieces inside it.

**2. Postgres is a dedicated CloudNativePG `Cluster`, matching the pattern Keycloak's already
...
Harbor's own bucket, not Image Factory's.** Harbor reuses the driver/region/endpoint/credential
resolution `config-object-store.yaml` already computes (`object_store.driver`, shared across
both consumers). It gets a second bucket (`${object_store.prefix}-harbor`), though, provisioned
by a second named `terraform:` stack entry against the same `object-store/hetzner`/`object-store/aws`
module Image Factory's own bucket uses. Not a shared bucket, and not a module change, for two
reasons. Harbor's registry component uses the same `distribution` storage layout Image Factory's
already does, and while blob paths are content-addressed (safe to collide), manifest and
...
settings for free: no widening the module's `list(string)` + shared-bool structure into a
per-bucket map.
...
OIDC is a post-boot REST call against the bootstrap admin session. `harbor/oidc` registers the
OIDC client in the platform realm, then configures Harbor over its own API, not a values
field. It's a named `oidc` resources variant of the `registry` flux system
(`registry-resources-oidc`, `dependsOn: [registry-install, identity-resources]`), mirroring how
`addon-identity.yaml` sequences its own `database` resources variant ahead of
the Keycloak CR, rather than a component of Harbor's install tier. That way Harbor's install
and gateway route come up regardless of whether identity is enabled. This is a deliberate
difference from Omni's own `omni/saml-client` Job, which *is* a component of `omni-install`
and accepts an unconditional `dependsOn: [identity-resources]` on the whole system: Omni has
no toggle to run without identity, Harbor does. `registry.harbor.sso` (default `true`) turns
SSO off explicitly if an operator wants Harbor on local-admin auth even with identity
enabled. Harbor's own local admin credential still exists either way, materialized like
every other bootstrap-admin secret in this repo. SSO adds a second login path; it
...
- Confirmed empirically while building this: a schema `default:` is not visible to another
  ...
  `database.postgres.driver ?? 'cloudnativepg'` for exactly this reason, matching what
...
- Trivy (vulnerability scanning) ships on by default with the chart, adding real CPU/memory
  footprint on top of core/portal/jobservice/registry/redis. It's sized the same
...
**A single shared bucket for both Image Factory and Harbor.** Rejected per decision 4: a
